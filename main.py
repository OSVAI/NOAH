...
from utils import Logger, AverageMeter, accuracy, mkdir_p, init_distributed_mode, get_dist_info, get_parameters
...
parser.add_argument('--nowd-bn', default=False, action='store_true',
...
#NOAH options
parser.add_argument('--use_noah', action="store_true", default=False)
parser.add_argument('--key_ratio', default=0.5, type=float)
...
def main():
    ...
    model = models.__dict__[args.arch](use_noah=args.use_noah, key_ratio=args.key_ratio,
                                       head_num=args.head_num, dropout=args.dropout, )

    # define loss function (criterion) and optimizer
    criterion = nn.CrossEntropyLoss().cuda()
    optimizer = optim.SGD(get_parameters(model) if args.nowd_bn else model.parameters(), args.lr,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_loss, train_acc = train(train_loader, train_loader_len, model, criterion,
                                      optimizer, use_cuda, scaler=scaler)
...
def train(train_loader, train_loader_len, model, criterion, optimizer, use_cuda, scaler=None):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        if batch_idx % args.print_freq == 0:
            print('({batch}/{size}) Data: {data:.3f}s | Batch: {bt:.3f}s | Loss: {loss:.4f} | top1: {top1: .4f} | '
                  'top5: {top5: .4f} | Total Time: {time: .2f}'.format(
                        batch=batch_idx + 1,
    ...
    return losses.avg.cpu().data, top1.avg.cpu().data
...
def adjust_learning_rate(optimizer, epoch, iteration, iter_per_epoch):
    ...
    if args.lr_decay == 'cos':
        ...
        lr = args.lr * (1 - current_iter / max_iter)
    else:
