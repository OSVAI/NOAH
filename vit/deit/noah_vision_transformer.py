...
class NOAH_VisionTransformer(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, head_split=True,
                 kv_split=True, key_ratio=0.5, noah_head_num=4, head_drop_rate=0.1):
        super().__init__()
    ...
    def reset_classifier(self, num_classes, global_pool=''):
        self.num_classes = num_classes
        self.head = NOAH(self.embed_dim, num_classes, head_split=True, kv_split=True, key_ratio=0.5, head_num=4,
                         dropout=self.head_drop_rate) if num_classes > 0 else nn.Identity()
    ...
    def forward(self, x):
        ...
        x = self.pre_logits(x)
        x = x.permute(0, 2, 1).unsqueeze(dim=3)
        x = self.head(x)
        return x
...
def vit_small_patch16_224(pretrained=False, **kwargs):
    ...
    model = NOAH_VisionTransformer(patch_size=16, embed_dim=768, depth=8, num_heads=8, mlp_ratio=3., **kwargs)
    return model


@register_model
def vit_base_patch16_224(pretrained=False, **kwargs):
    model = NOAH_VisionTransformer(
...
def vit_huge_patch16_224(pretrained=False, **kwargs):
    model = NOAH_VisionTransformer(patch_size=16, embed_dim=1280, depth=32, num_heads=16, mlp_ratio=4, **kwargs)
    return model
